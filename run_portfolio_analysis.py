...
def test_balance_generator():
    ...
    print("[OK] Balance generator tests passed")
...
def test_portfolio_generator():
    ...
    print("[OK] Portfolio generator tests passed")
...
def generate_and_report_statistics():
    ...
    for product in product_order:
        if product in statistics:
            ...
            min_compliance = "[OK]" if stats['min'] >= config['min'] else "[FAIL]"
            max_compliance = "[OK]" if stats['max'] <= config['max'] else "[FAIL]"
    ...
    for product, is_right_skewed in validation['right_skewness_by_product'].items():
        status = "[OK]" if is_right_skewed else "[FAIL]"
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("[OK] Successfully generated 10,000-loan base portfolio")
    print("[OK] All validation tests passed")
    print("[OK] Statistics reported for all products")
    print("[OK] Methodological considerations documented")
